...
cmd:option('-tag_weight',0.0, [[Weight to add to diff tags]])
      ...
      for b = 1, #results do
        if (#srcBatch[b].words == 0) then
          ...
          if withGoldScore then
            ...
            _G.logger:info("GOLD SCORE: %.64f", results[b].goldScore)
          ...
          for n = 1, #results[b].preds do
            ...
            if #results[b].preds > 1 then
              _G.logger:info("[%.64f] %s", results[b].preds[n].score, sentence)
            else
              _G.logger:info("PRED %d: %s", sentId, sentence)
              _G.logger:info("PRED SCORE: %.64f", results[b].preds[n].score)
            end
          end
        end
